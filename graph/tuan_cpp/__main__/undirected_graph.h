...
private:
  ...
  static void __depthFirstTraverse(
    ConstVertexCallBack<T>& cb, const std::vector<UndirectedEdge<T>*> vecUE,
...
public:
  ...
  UndirectedGraph(std::string k, T* v) {
  ...
  const std::vector<UndirectedEdge<T>*> getEdges() const {
    std::vector<UndirectedEdge<T>*> v;
  ...
  void presentMatrix() {
    ...
    for(auto& v : __vertices) {
      ...
      for(auto& v2 : __vertices) {
        if(__edges.find(new UndirectedEdge<T>(v.second, v2.second, 0)) != __edges.end()) std::cout << "1 ";
        else std::cout << "0 ";
      }
      std::cout << std::endl;
    };
  ...
  void presentList() {
    for(auto& v : __vertices) {
      ...
      for(auto& e : __edges) {
        ...
        if(e->second->is(v.second->getKey())) std::cout << e->first->getKey() << " ";
      };
      std::cout << std::endl;
    };
  };
...
#endif // UNDIRECTED_GRAPH_H_INCLUDED
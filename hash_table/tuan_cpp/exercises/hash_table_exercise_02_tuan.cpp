...
public:
  ...
  Bucket() = default;
  Bucket(string k, T* v): key{k}, value{v} {};
...
class HashTable {
private:
  vector<vector<Bucket<T>>> __data = vector<vector<Bucket<T>>>(10);

  int __getIndexOfBucket(string key) {
...
public:
  HashTable() = default;
  HashTable(size_t s) {
    this->__data = vector<vector<Bucket<T>>>(10);
  };
  ...
  void forEach(ConstBucketCallBack<T> cb) {
    typename vector<vector<Bucket<T>>>::iterator itr = __data.begin();
    typename vector<vector<Bucket<T>>>::iterator end_itr = __data.end();
    int c = 0;
    while(itr != end_itr) {
      if((*itr).size() == 0) {
        c++;
      ...
      typename vector<Bucket<T>>::iterator sub_itr = (*itr).begin();
      typename vector<Bucket<T>>::iterator sub_end_itr = (*itr).end();
      cout << "Bucket list [" << c << "]: \n";
